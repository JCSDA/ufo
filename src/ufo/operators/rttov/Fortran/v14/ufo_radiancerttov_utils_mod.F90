    ...
    if(conf % SatRad_compatibility) then
      ...
      do iprof = 1, nProfiles
        ...
        if(conf % BoundQToSaturation) then
          ...
          if (profiles(iprof) % near_surface(1) % q2m > qsaturated(1)) &
...
!Set RTTOV viewing geometry
  ...
  subroutine ufo_rttov_alloc_profiles(self, errorstatus, conf, nprofiles, nlevels, init, asw)
    ...
    integer                            :: nsurfaces = 1 ! This is hard-coded to 1 until the full capability to
    ...
    do iprof=1, size(prof_list, dim=1)
      ...
      if (self % profiles(all_prof_index) % skin(1) % surftype == surftype_sea) then
        ...
        if (conf % rttov_coef_array(1) % coef % sensor_type == sensor_id_mw) then
          ...
          elseif (self % profiles(all_prof_index) % skin(1) % surftype == surftype_seaice) then
            self % emissivity(1) % emis_in(start_chan:end_chan) = 0.92_kind_real
          end if

        elseif ( conf % rttov_coef_array(1) % coef % sensor_type == sensor_id_ir .or. &
