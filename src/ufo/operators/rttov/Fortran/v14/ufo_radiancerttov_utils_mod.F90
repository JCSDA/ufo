...
module ufo_radiancerttov_utils_mod
  ...
  use rttov_types, only : rttov_options, rttov_profile, rttov_coefs, &
    rttov_radiance, rttov_transmission, rttov_emis_refl, rttov_chanprof
  ...
  use rttov_const, only : gas_id_mixed, gas_id_watervapour, gas_id_ozone, gas_id_wvcont, gas_id_co2, &
    ...
    gas_name, &
    mair, mh2o, mo3, mco2, mn2o, mco, mch4, mso2, &
    errorstatus_success, errorstatus_fatal, &
    interp_rochon_wfn, thermal_solver_delta_edd, solar_solver_dom, clw_perm_rosenkranz, &
    solar_refl_model_elfouhaily, ir_emis_model_iremis, mw_emis_model_fastem6, mw_emis_model_surfem_ocean, &
    ...
    gas_unit_ppmvdry, gas_unit_kg_per_kg, &
  ...
  character(len=maxvarlen), dimension(9), public :: varin_default = &
    [var_prsi, var_prs, var_ts, var_sfc_t2m, &
    var_sfc_u10, var_sfc_v10, var_ps, var_sfc_q2m, &
    var_sfc_tskin ]

  character(len=maxvarlen), dimension(3), public :: varin_scatt = &
    [var_clw, var_cli, var_cldfrac_vol ]
  ...
  ! Convenient array of dry gas conversion ratios
  real(kind=kind_real), parameter :: mixratio_to_ppmv(ngases_max) = &
    [ 1.0_kind_real,                  & ! mixed gases not used
       (mair / mh2o) * 1e+6_kind_real, & ! WV
       (mair / mo3)  * 1e+6_kind_real, & ! Ozone
       1.0_kind_real,                  & ! WV continuum not used externally
       (mair / mco2) * 1e+6_kind_real, & ! CO2
       (mair / mn2o) * 1e+6_kind_real, & ! N2O
       (mair / mco)  * 1e+6_kind_real, & ! CO
       (mair / mch4) * 1e+6_kind_real, & ! CH4
       (mair / mso2) * 1e+6_kind_real ] ! SO2
  ...
  type, public :: ufo_rttov_io
    ...
    type(rttov_emis_refl), pointer   :: emissivity(:)   ! Input/output surface emissivity
    ...
    type(rttov_emis_refl), pointer   :: emissivity_k(:) ! Input/output surface emissivity
    type(rttov_transmission)         :: transmission_k  ! Output transmittances
    type(rttov_radiance)             :: radiance_k      ! Output radiances

    real(kind_real), allocatable     :: ciw(:,:)                ! pointer to either ice from MW scattering or diagnosed ice from Qsplit
  ...
  type rttov_conf
    ...
    integer                               :: rttov_hydro_index_clw
    integer                               :: rttov_hydro_index_ciw
    character(len=255)                    :: rttov_hydro_name_clw
    character(len=255)                    :: rttov_hydro_name_ciw
...
contains
  ...
  subroutine rttov_conf_setup(conf, f_confOpts, f_confOper)
    ...
    if (f_confOper%has(trim(absorber_name))) then
    ...
    call f_confOpts % get_or_die("MW_Scatt_hydrometeor_name_clw", str)
    conf % rttov_hydro_name_clw = trim(str)
    call f_confOpts % get_or_die("MW_Scatt_hydrometeor_name_ciw", str)
    conf % rttov_hydro_name_ciw = trim(str)
    ...
    if(conf % do_mw_scatt) then
      conf % rttov_opts % scatt % hydrometeors = .true.
    end if

    if (conf % do_mw_scatt .and. .not. conf % prof_by_prof) then
      message = "MW scattering does not support batch processing. Setting prof_by_prof to TRUE"
      call oops_log%trace(message)
      conf % prof_by_prof = .true.
    end if
    ...
    if (any(conf % rttov_coef_array(:) % coef % sensor_type == sensor_id_mw)) then
      ...
      conf % rttov_opts % rt_all % o3_data = .false.
      conf % rttov_opts % rt_all % co2_data = .false.
      conf % rttov_opts % rt_all % n2o_data = .false.
      conf % rttov_opts % rt_all % ch4_data = .false.
      conf % rttov_opts % rt_all % co_data = .false.
      conf % rttov_opts % rt_all % so2_data = .false.
    end if

    ! Ensure the RTTOV options and coefficients are consistent
    call rttov_user_check_options( rttov_errorstatus,          &
                                   conf % rttov_opts,          &
  ...
  subroutine rttov_conf_delete(conf)
    ...
    if (allocated(conf % rttov_coef_array)) then
      do i = 1, size(conf % rttov_coef_array)
        call rttov_dealloc_coefs( rttov_errorstatus,          &
                                  conf % rttov_coef_array(i) )
      end do
  ...
  subroutine ufo_rttov_set_options(self, f_confOpts)
    ...
    if (f_confOpts % has("RTTOV_addrefrac")) then
      call f_confOpts % get_or_die("RTTOV_addrefrac", self % rttov_opts % rt_all % refraction)
    end if

    !< Switch for input units in AD/K models
    if (f_confOpts % has("RTTOV_switchrad")) then
      call f_confOpts % get_or_die("RTTOV_switchrad", self % rttov_opts % config % adk_bt)
    end if
    ...
    !< Switch for setting Lambertian reflection
    if (f_confOpts % has("RTTOV_do_lambertian")) then
      call f_confOpts % get_or_die("RTTOV_do_lambertian", self % rttov_opts % surface % lambertian)
    end if

    !< Switch for fixed/parameterised effective angle for Lambertian option
    if (f_confOpts % has("RTTOV_lambertian_fixed_angle")) then
      call f_confOpts % get_or_die("RTTOV_lambertian_fixed_angle", self % rttov_opts % surface % lambertian_fixed_angle)
    end if
    ...
    !< MW sea emissivity model. For v14.x: 1=FASTEM-5, 2=FASTEM-6 3=>SURFEM-Ocean
    ! Note that this is significantly different from v12 and options have been both re-numbered and removed.
    if (f_confOpts % has("RTTOV_fastem_version")) then
      call f_confOpts % get_or_die("RTTOV_fastem_version", self % rttov_opts % surface % mw_sea_emis_model)
    end if

    !< Supply a foam fraction to FASTEM
    if (f_confOpts % has("RTTOV_supply_foam_fraction")) then
      call f_confOpts % get_or_die("RTTOV_supply_foam_fraction", self % rttov_opts % surface % use_foam_fraction)
    end if

    !< Switch to enable input of cloud liquid water profile
    if (f_confOpts % has("RTTOV_clw_data")) then
      call f_confOpts % get_or_die("RTTOV_clw_data", self % rttov_opts % clw_absorption % clw_data)
    end if

    !< MW CLW scheme
    if (f_confOpts % has("RTTOV_clw_scheme")) then
      call f_confOpts % get_or_die("RTTOV_clw_scheme", self % rttov_opts % clw_absorption % permittivity_param)
    end if

    !< Lower pressure limit for MW CLW calculations (hPa)
    if (f_confOpts % has("RTTOV_clw_cloud_top")) then
      call f_confOpts % get_or_die("RTTOV_clw_cloud_top", self % rttov_opts % clw_absorption % clw_cloud_top)
    end if

    !< Switch to enable RTTOV interpolator
    if (f_confOpts % has("RTTOV_addinterp")) then
      call f_confOpts % get_or_die("RTTOV_addinterp", self % rttov_opts % interpolation % enable_interp)
    end if
    ...
    if (f_confOpts % has("RTTOV_lgradp")) then
      call f_confOpts % get_or_die("RTTOV_lgradp", self % rttov_opts % interpolation % pressure_gradients)
    end if
    ...
    if (f_confOpts % has("RTTOV_do_checkinput")) then
      call f_confOpts % get_or_die("RTTOV_do_checkinput", self % rttov_opts % config % check_profiles)
    end if
    ...
    !< Solar sea BRDF model
    if (f_confOpts % has("RTTOV_solar_sea_brdf_model")) then
      call f_confOpts % get_or_die("RTTOV_solar_sea_brdf_model", self % rttov_opts % surface % solar_sea_refl_model)
    end if

    !< IR sea emissivity model
    if (f_confOpts % has("RTTOV_ir_sea_emis_model")) then
      call f_confOpts % get_or_die("RTTOV_ir_sea_emis_model", self % rttov_opts % surface % ir_sea_emis_model)
    end if

    !< Switch to enable solar simulations
    if (f_confOpts % has("RTTOV_addsolar")) then
      call f_confOpts % get_or_die("RTTOV_addsolar", self % rttov_opts % rt_all % solar)
    end if

    !< Switch to enable Rayleigh single-scattering for VIS/NIR channel
    if (f_confOpts % has("RTTOV_rayleigh_single_scatt")) then
      call f_confOpts % get_or_die("RTTOV_rayleigh_single_scatt", self % rttov_opts % rt_all % rayleigh_single_scatt)
    end if

    !< Switch to enable NLTE bias correction
    if (f_confOpts % has("RTTOV_do_nlte_correction")) then
      call f_confOpts % get_or_die("RTTOV_do_nlte_correction", self % rttov_opts % rt_all % nlte_correction)
    end if

    !< Switch to enable IR aerosol calculations
    if (f_confOpts % has("RTTOV_addaerosl")) then
      call f_confOpts % get_or_die("RTTOV_addaerosl", self % rttov_opts % scatt % aerosols)
    end if

    !< Switch to supply aerosol optical properties explicitly per channel
    if (f_confOpts % has("RTTOV_user_aer_opt_param")) then
      call f_confOpts % get_or_die("RTTOV_user_aer_opt_param", self % rttov_opts % scatt % user_aer_opt_param)
    end if

    if (f_confOpts % has("Do_MW_Scatt")) then
      call f_confOpts % get_or_die("Do_MW_Scatt", self % rttov_opts % scatt % hydrometeors)
    end if

    !< Switch to supply cloud optical properties explicitly per channel
    if (f_confOpts % has("RTTOV_user_cld_opt_param")) then
      call f_confOpts % get_or_die("RTTOV_user_cld_opt_param", self % rttov_opts % scatt % user_hydro_opt_param)
    end if

    !< Effective cloud column weight threshold for determining if scattering calculations will be performed
    if (f_confOpts % has("RTTOV_cldstr_threshold")) then
      call f_confOpts % get_or_die("RTTOV_cldstr_threshold", self % rttov_opts % cloud_overlap % col_threshold)
    end if

    !< Cloud overlap parameterisation
    if (f_confOpts % has("RTTOV_cloud_overlap_param")) then
      call f_confOpts % get_or_die("RTTOV_cloud_overlap_param", self % rttov_opts % cloud_overlap % overlap_param)
    end if

    !< Upper pressure limit for two_col_max_frac option (hPa)
    if (f_confOpts % has("RTTOV_cldstr_low_cloud_top")) then
      call f_confOpts % get_or_die("RTTOV_cldstr_low_cloud_top", self % rttov_opts % cloud_overlap % two_col_max_frac_max_p)
    end if

    !< IR scattering model to use
    if (f_confOpts % has("RTTOV_ir_scatt_model")) then
      call f_confOpts % get_or_die("RTTOV_ir_scatt_model", self % rttov_opts % scatt % thermal_solver)
    end if

    !< VIS/NIR scattering model to use
    if (f_confOpts % has("RTTOV_vis_scatt_model")) then
      call f_confOpts % get_or_die("RTTOV_vis_scatt_model", self % rttov_opts % scatt % solar_solver)
    end if

    !< Number of DOM streams, must be even and not less than 2
    if (f_confOpts % has("RTTOV_dom_nstreams")) then
      call f_confOpts % get_or_die("RTTOV_dom_nstreams", self % rttov_opts % scatt % dom_nstreams)
    end if

    !< Convergence criterion for termination of DOM azimuthal loop
    if (f_confOpts % has("RTTOV_dom_accuracy")) then
      call f_confOpts % get_or_die("RTTOV_dom_accuracy", self % rttov_opts % scatt % dom_accuracy)
    end if

    !< DOM ignores levels below this optical depth:
    if (f_confOpts % has("RTTOV_dom_opdep_threshold")) then
      call f_confOpts % get_or_die("RTTOV_dom_opdep_threshold", self % rttov_opts % scatt % dom_opdep_threshold)
    end if

    !< Switch to enable input of O3 profile
    if (f_confOpts % has("RTTOV_ozone_data")) then
      call f_confOpts % get_or_die("RTTOV_ozone_data", self % rttov_opts % rt_all % o3_data)
    end if

    !< Switch to enable input of CO2 profile
    if (f_confOpts % has("RTTOV_co2_data")) then
      call f_confOpts % get_or_die("RTTOV_co2_data", self % rttov_opts % rt_all % co2_data)
    end if

    !< Switch to enable input of N2O profile
    if (f_confOpts % has("RTTOV_n2o_data")) then
      call f_confOpts % get_or_die("RTTOV_n2o_data", self % rttov_opts % rt_all % n2o_data)
    end if

    !< Switch to enable input of CO profile
    if (f_confOpts % has("RTTOV_co_data")) then
      call f_confOpts % get_or_die("RTTOV_co_data", self % rttov_opts % rt_all % co_data)
    end if

    !< Switch to enable input of CH4 profile
    if (f_confOpts % has("RTTOV_ch4_data")) then
      call f_confOpts % get_or_die("RTTOV_ch4_data", self % rttov_opts % rt_all % ch4_data)
    end if

    !< Switch to enable input of SO2 profile
    if (f_confOpts % has("RTTOV_so2_data")) then
      call f_confOpts % get_or_die("RTTOV_so2_data", self % rttov_opts % rt_all % so2_data)
    end if
    ...
    if (.not. self%rttov_is_setup ) then
      ...
      do i_inst = 1, self%nSensors
        ...
        call rttov_read_coefs(rttov_errorstatus,                                  & ! out
                              ...
                              self % rttov_opts,                                  & ! in
                              instrument = self % instrument_triplet(1:3,i_inst), & ! in
                              path = trim(self % COEFFICIENT_PATH))                 ! in
        ...
        if(self % rttov_opts % scatt % hydrometeors) then

          self % rttov_hydro_index_clw = &
            find_hydrotable_index(self % rttov_coef_array(i_inst), self % rttov_hydro_name_clw)
          self % rttov_hydro_index_ciw = &
            find_hydrotable_index(self % rttov_coef_array(i_inst), self % rttov_hydro_name_ciw)

          if( self % rttov_hydro_index_clw /= -1 .AND. self % rttov_hydro_index_ciw /= -1) THEN
            write(message,"(A,I0)") "Cloud Liquid Water hydrotable index: ", self % rttov_hydro_index_clw
            call oops_log%trace(message)
            write(message,"(A,I0)") "Cloud Ice Water hydrotable index: ", self % rttov_hydro_index_ciw
            call oops_log%trace(message)
          else
            message = "Error finding hydrometeor table index for CLW and/or CIW. Check names are correct: " // &
              trim(self % rttov_hydro_name_clw) // ", " // trim(self % rttov_hydro_name_ciw)
            call abor1_ftn(message)
          end if
        end if
      end do
  ...
  subroutine ufo_rttov_setup_rtprof(self,geovals,obss,conf,ob_info)
    ...
    real(kind_real), allocatable       :: t_temp(:), q_temp(:), clw_temp(:), ciw_temp(:), Qtotal(:), qsaturated(:)
    real(kind_real)                    :: scale_fac
    real(c_double)                     :: missing

    integer                            :: year, month, day, hour, minute, second

    integer, allocatable               :: sat_id(:), surf_temp(:)
    ...
    if (ufo_vars_getindex(geovals%variables, var_prsi) > 0 ) then
      ...
      do iprof = 1, nprofiles
        ...
        if (ph(1) > ph(2)) then !upside-down
          profiles(iprof) % p_half(top_level+1:bottom_level:stride) = ph(:)
        else
          profiles(iprof) % p_half(top_level:bottom_level+1:stride) = ph(:)
        end if
        if( conf % debug) then
          write(*,*) "profiles(iprof) % p_half(1): ", profiles(1) % p_half(1), RTTOV_ToA
        end if
        profiles(iprof) % p_half(1) = RTTOV_ToA ! hard-code top of atmosphere 0.0001 hPa
...
! Get absorbers
    if (conf % RTTOV_GasUnitConv) then
      ...
      profiles(1:nprofiles)%gas_units = gas_unit_kg_per_kg
    end if
    ...
    do jspec = 1, conf%ngas
      ...
      case (var_cli)
        ...
        if (conf % do_mw_scatt) then
          do iprof = 1, nProfiles
            profiles(iprof) % hydro(conf % rttov_hydro_index_ciw, top_level:bottom_level:stride) = geoval%vals(:, iprof)
          end do
        end if
      case default
        message = "ERROR: " // trim(conf%Absorbers(jspec)) // &
                  " not setup to be read from GeoVaLs => Aborting"
        call abor1_ftn(message)
      end select

    end do

! Get near-surface variables

    varname = var_ps
    if (ufo_vars_getindex(geovals%variables, varname) > 0) then
      call ufo_geovals_get_var(geovals, varname, geoval)
      do iprof = 1, nprofiles
        profiles(iprof)%p_half(nlevels+1) = geoval%vals(1,iprof) * Pa_to_hPa
      end do
    end if
    ...
    if (ufo_vars_getindex(geovals%variables, varname) > 0) then
      call ufo_geovals_get_var(geovals, varname, geoval)
      do iprof = 1, nprofiles
        profiles(iprof) % near_surface(1) % t2m = geoval%vals(1,iprof)
      end do
    else
      message = "No near-surface temperature. Using bottom temperature level"
      call oops_log%trace(message)
      do iprof = 1, nprofiles
        profiles(iprof) % near_surface(1) % t2m = profiles(iprof)%t(nlevels)
      end do
    end if
    ...
    if (ufo_vars_getindex(geovals%variables, varname) > 0) then
      ...
      do iprof = 1, nprofiles
        profiles(iprof)%near_surface(1) % q2m = geoval%vals(1,iprof) * conf%scale_fac(gas_id_watervapour)
      end do
    else
      ...
      do iprof = 1, nprofiles
        profiles(iprof)%near_surface(1) % q2m = profiles(iprof)%q(nlevels) * conf%scale_fac(gas_id_watervapour)
      end do
    end if
    ...
    if (ufo_vars_getindex(geovals%variables, varname) > 0) then
      call ufo_geovals_get_var(geovals, varname, geoval)
      do iprof = 1, nprofiles
        profiles(iprof)%near_surface(1) % wind_u10m = geoval%vals(1,iprof)
      end do
      ...
      call ufo_geovals_get_var(geovals, varname, geoval)
      do iprof = 1, nprofiles
        profiles(iprof)%near_surface(1) % wind_v10m = geoval%vals(1,iprof)
      end do
    ...
    if (skinTIndexFromObsSpace > 0) then
      ...
      do iprof = 1, nprofiles
        profiles(iprof) % skin(1) % t = geoval%vals(1,iprof)
      end do
    ...
    do iprof = 1, size(profiles)
      if (profiles(iprof) % ctp > profiles(iprof) % p_half(nlevels + 1)) then
        profiles(iprof) % ctp = profiles(iprof) % p_half(nlevels + 1)
      end if
    end do
   ...
   if (conf % do_mw_scatt) then
     ...
     do iprof = 1, nprofiles
       profiles(iprof) % hydro_frac(1, top_level:bottom_level:stride) = &
         geoval%vals(:, iprof)
     end do
   end if
...
! ---------------------------
    ...
    if(conf % SatRad_compatibility) then
      ...
      do iprof = 1, nProfiles
        ...
        if(conf % BoundQToSaturation) then
          ...
          s2m_t(1) = profiles(iprof) % near_surface(1) % t2m
          s2m_p(1) = profiles(iprof) % p_half(nlevels + 1) / Pa_to_hPa
          if (conf % UseRHwaterForQC) then
          ...
          if (profiles(iprof) % near_surface(1) % q2m > qsaturated(1)) then
            profiles(iprof) % near_surface(1) % q2m = qsaturated(1)
          end if
          deallocate(qsaturated)
        end if
        ...
        if(conf % UseMinimumQ) then
          ...
          if(profiles(iprof) % near_surface(1) % q2m < min_q * conf%scale_fac(gas_id_watervapour)) then
             profiles(iprof) % near_surface(1) % q2m = min_q * conf%scale_fac(gas_id_watervapour)
          end if
        end if
        ...
        if(conf % UseMinimumClw) then
          if (conf % do_mw_scatt) then
            ...
            do ilev = 1, nlevels
              if (profiles(iprof) % hydro(conf % rttov_hydro_index_clw,ilev) < min_clw) then
                profiles(iprof) % hydro(conf % rttov_hydro_index_clw,ilev) = min_clw
        ...
        if(conf % UseMinimumCiw .and. conf % do_mw_scatt) then
          ! Cloud Ice Water only supported for MW scattering
          ! Constrain small values to min_ciw for totalice profile
          do ilev = 1, nlevels
            if (profiles(iprof) % hydro(conf % rttov_hydro_index_ciw,ilev) < min_ciw) then
              profiles(iprof) % hydro(conf % rttov_hydro_index_ciw,ilev) = min_ciw
    ...
    if(conf % SplitQtotal) then
      ...
      if (conf % do_mw_scatt) then
        do iprof = 1, nprofiles
          self % ciw(:,iprof) = profiles(iprof) % hydro(conf % rttov_hydro_index_ciw, :)
        end do
      end if
    end if
...
! Set geometry for RTTOV calculation, either from the supplied ob info (1dvar) or the obsspace db
    if(present(ob_info)) then
      ...
      profiles(1) % skin(1) % surftype = ob_info % surface_type
...
!lat/lon
      ...
      if (variable_present) then
        ...
        message = "Warning: Optional input MetaData/latitude not in database: setting to zero for RTTOV"
        call oops_log%trace(message)
        profiles(1:nprofiles)%latitude = zero
      end if
...
!Set RTTOV viewing geometry
      ...
      if (variable_present) then
        allocate (surf_temp(nprofiles))
        call obsspace_get_db(obss, "MetaData", var_surf_type_rttov, surf_temp)
        do iprof = 1, nprofiles
          profiles(iprof) % skin(1) % surftype = surf_temp(iprof)
        end do
  ...
  subroutine ufo_rttov_check_rtprof(self, conf, iprof, errorstatus)
    ...
    include 'rttov_user_check_profile.interface'
    ...
    if (sensor_idx < 1) then
      ...
      if(conf % RTTOV_profile_checkinput) then
        call rttov_user_check_profile( errorstatus,                         &
                                       conf % rttov_opts,                   &
                                       conf % rttov_coef_array(sensor_idx), &
                                       self % profiles(iprof) )

        if (self % profiles(iprof) % ctp < zero .or. &
            self % profiles(iprof) % ctp > maxval(self % profiles(iprof) % p_half(:))) then

          errorstatus = errorstatus_fatal
          write(message, "(A,I0,A)") "Cloud top pressure is out of bounds for profile ", iprof, &
        ...
        if(errorstatus /= errorstatus_success .and. conf % debug) then
          ...
          call rttov_print_profile( self % profiles(iprof), &
                                    lu = stderr )
          write(message, "(A,I0)") "Error in profile ", iprof
          call oops_log%trace(message)
        end if

        if ((conf % rttov_coef_array(sensor_idx) % coef % sensor_type == sensor_id_mw) .and. &
            (self % profiles(iprof) % azangle < zero .or. &
             self % profiles(iprof) % azangle > 360.0_kind_real)) then
          errorstatus = errorstatus_fatal
          write(message, "(A,I0,A)") "Bad azimuth angle which is required for MW sensors ", &
  ...
  !ufo_rttov_alloc is a wrapper for RTTOV 14 allocation
  subroutine ufo_rttov_alloc_direct(self, errorstatus, conf, nprofiles, nchannels, nlevels, init, asw)
    ...
    logical,             intent(in)    :: asw ! allocate switch: .true. to allocate, .false. to deallocate
    ...
    if(asw) then
      ! Initialise emissivity so that if emissivity is not set then it is an error
      self % emissivity(1) % calc_emis = .false.
      self % emissivity(1) % emis_in = -one
      self % emissivity(1) % emis_out = -one
    else
  ...
  subroutine ufo_rttov_alloc_k(self, errorstatus, conf, nprofiles, nchannels, nlevels, init, asw)
    ...
    call rttov_alloc_k( errorstatus,                            &
                        asw,                                    &
                        nchannels,                              &
                        nprofiles,                              &
                        nlevels+1,                              &
  ...
  subroutine ufo_rttov_alloc_profiles(self, errorstatus, conf, nprofiles, nlevels, init, asw)
    ...
    integer, parameter                 :: nsurfaces = 1 ! This is hard-coded to 1 until the full capability to
                                                        ! handle multiple surfaces is added to the interface

    include 'rttov_alloc_profiles.interface'
    ...
    if (asw) allocate (self % profiles(nprofiles))

    ! Allocate structures for rttov_direct
    call rttov_alloc_profiles( errorstatus,                        &
                               asw,                                &
                               self % profiles,                    &
                               nlevels+1,                          &
                               nsurfaces,                          &
    ...
    if (asw) then
      do iprof = 1, nprofiles
        self % profiles(iprof) % skin(1) % watertype = watertype_ocean_water
        self % profiles(iprof) % near_surface(1) % wind_fetch = 100000.0_kind_real ! wind fetch (m) taken
                                                                                   ! from users guide
        self % profiles(iprof) % skin(1) % salinity = 35.0_kind_real

      !Default fastem parameters
        self % profiles(iprof) % skin(1) % fastem = [3.0_kind_real,  &
                                                  5.0_kind_real,  &
                                                  15.0_kind_real, &
                                                  0.1_kind_real,  &
                                                  0.3_kind_real]
      ...
      ! set surftype to invalid so that it must be set explicitly otherwise an error will occur
        self % profiles(iprof) % skin(1) % surftype = -1
      end do
    else
  ...
  subroutine ufo_rttov_zero_k(self, conf, reset_profiles_k)
    ...
    include 'rttov_init_profiles.interface'
    include 'rttov_init_radiance.interface'
    include 'rttov_init_transmission.interface'

    if (reset_profiles_k) then
      call rttov_init_profiles( self % profiles_k )

    end if

    call rttov_init_radiance( self % radiance_k )
    call rttov_init_transmission( self % transmission_k )
    call rttov_init_emis_refl( self % emissivity_k )
...
! Set radiance jacobian scaling (should equal one)
  ...
  subroutine ufo_rttov_init_atlas_emissivity(self, conf, RTTOV_Atlas_Emissivity, chanprof, &
    ...
    do iprof=1, size(prof_list, dim=1)
      ...
      if (self % profiles(all_prof_index) % skin(1) % surftype == surftype_sea) then

        ! Calculate by RTTOV
        self % emissivity(1) % emis_in(start_chan:end_chan) = zero
        self % emissivity(1) % calc_emis(start_chan:end_chan) = .true.

      else

        ! Use atlas emissivity
        self % emissivity(1) % emis_in(start_chan:end_chan) = RTTOV_Atlas_Emissivity(start_chan:end_chan)
        self % emissivity(1) % calc_emis(start_chan:end_chan) = .false.
...
!cycle through the list of good profiles
    ...
    do iprof=1, size(prof_list, dim=1)
      ...
      if (self % profiles(all_prof_index) % skin(1) % surftype == surftype_sea) then
        ...
        if (conf % rttov_coef_array(1) % coef % sensor_type == sensor_id_mw) then

          if (self % profiles(all_prof_index) % skin(1) % surftype == surftype_land) then
            self % emissivity(1) % emis_in(start_chan:end_chan) = 0.95_kind_real
          else if (self % profiles(all_prof_index) % skin(1) % surftype == surftype_seaice) then
            self % emissivity(1) % emis_in(start_chan:end_chan) = 0.92_kind_real
          end if

        else if ( conf % rttov_coef_array(1) % coef % sensor_type == sensor_id_ir .or. &
                 conf % rttov_coef_array(1) % coef % sensor_type == sensor_id_hi) then
          self % emissivity(1) % emis_in(start_chan:end_chan) = 0.98_kind_real
  ...
  subroutine ufo_rttov_set_defaults(self, default_opts_set)
    ...
    !Set RTTOV 14 defaults explicitly
    self % rttov_opts % cloud_overlap % col_threshold          = zero           !< Ignore cloud streams with weights lower than this
    self % rttov_opts % cloud_overlap % hydro_frac_tlad        = .true.         !< Switch for hydrometeor TL/AD sensitivity to effective cfrac
    self % rttov_opts % cloud_overlap % overlap_param          = 0              !< Cloud overlap parameter (0=auto)
    self % rttov_opts % cloud_overlap % two_col_max_frac_max_p = 750._kind_real !< Upper pressure limit for cldstr_simple option (hPa)

    self % rttov_opts % clw_absorption % clw_cloud_top         = 322                 !< Lower pressure limit for MW CLW calculations (hPa)
    self % rttov_opts % clw_absorption % clw_data              = .false.             !< Switch to enable input of cloud liquid water profile
    self % rttov_opts % clw_absorption % permittivity_param    = clw_perm_rosenkranz !< MW CLW scheme: 1 => Liebe, 2 => Rosenkranz, 3 => TKC

    self % rttov_opts % config % adk_bt                        = .true.  !< Switch for input units in AD/K models
    self % rttov_opts % config % apply_reg_limits              = .false. !< Switch to restrict input profiles to coef training limits
    self % rttov_opts % config % check_profiles                = .true.  !< Switch to apply internal profile checking
    self % rttov_opts % config % verbose                       = .false. !< Switch for verbose output

    self % rttov_opts % interpolation % enable_interp          = .true.            !< Switch to enable RTTOV interpolator
    self % rttov_opts % interpolation % interp_mode            = interp_rochon_wfn !< Interpolation mode 4 (valid options 1-5, see user guide)
    self % rttov_opts % interpolation % pressure_gradients     = .false.           !< Switch to make pressure an active variable in TL/AD/K models

    self % rttov_opts % rt_all % o3_data                       = .false. !< Switch to enable input of O3 profile
    self % rttov_opts % rt_all % co2_data                      = .false. !< Switch to enable input of CO2 profile
    self % rttov_opts % rt_all % n2o_data                      = .false. !< Switch to enable input of N2O profile
    self % rttov_opts % rt_all % co_data                       = .false. !< Switch to enable input of CO profile
    self % rttov_opts % rt_all % ch4_data                      = .false. !< Switch to enable input of CH4 profile
    self % rttov_opts % rt_all % so2_data                      = .false. !< Switch to enable input of SO2 profile

    self % rttov_opts % rt_all % nlte_correction               = .false. !< Switch to enable NLTE bias correction
    ...
    self % rttov_opts % rt_all % rayleigh_single_scatt         = .true.  !< Switch to enable Rayleigh single-scattering for VIS/NIR channels
    self % rttov_opts % rt_all % refraction                    = .true.  !< Switch to enable atmospheric refraction
    self % rttov_opts % rt_all % solar                         = .false. !< Switch to enable solar simulations
    self % rttov_opts % rt_all % use_q2m                       = .true.  !< Switch to enable use of 2m q variable
    self % rttov_opts % rt_all % use_t2m                       = .true.  !< Switch to enable use of 2m t variable

    self % rttov_opts % scatt % aerosols                       = .false.                  !< Switch to enable IR aerosol calculations
    self % rttov_opts % scatt % dom_accuracy                   = 0._kind_real             !< Convergence criterion for termination of DOM azimuthal loop
    self % rttov_opts % scatt % dom_nstreams                   = 8                        !< Number of DOM streams, must be even and not less than 2
    self % rttov_opts % scatt % dom_opdep_threshold            = 0._kind_real             !< DOM ignores levels below this optical depth:
    self % rttov_opts % scatt % hydrometeors                   = .false.                  !< Switch to enable cloudy calculations
    self % rttov_opts % scatt % solar_solver                   = solar_solver_dom         !< VIS/NIR scattering model to use
    self % rttov_opts % scatt % thermal_solver                 = thermal_solver_delta_edd !< IR scattering model to use
    self % rttov_opts % scatt % user_aer_opt_param             = .false.                  !< Switch to supply aerosol optical properties explicitly per channel
    self % rttov_opts % scatt % user_hydro_opt_param           = .false.                  !< Switch to supply cloud optical properties explicitly per channel
    self % rttov_opts % scatt % zero_hydro_tlad                = .false.                  !< Switch for hydrometeor TL/AD sensitivity in layers with zero hydrometeor concentration

    self % rttov_opts % surface % ir_sea_emis_model            = ir_emis_model_iremis        !< IR sea emissivity model
    self % rttov_opts % surface % lambertian                   = .false.                     !< Switch for setting Lambertian reflection (IR and MW)
    self % rttov_opts % surface % lambertian_fixed_angle       = .true.                      !< Switch for fixed/parameterised effective angle for Lambertian option
    self % rttov_opts % surface % mw_sea_emis_model            = mw_emis_model_surfem_ocean  !< Select MW emissivity model for ocean surfaces
    self % rttov_opts % surface % solar_sea_refl_model         = solar_refl_model_elfouhaily !< Solar sea BRDF model
    self % rttov_opts % surface % use_foam_fraction            = .false.                     !< Supply a foam fraction to the MW emissivity model for ocean surfaces

! Set options for Met Office specific PS configurations
    if (PS_configuration) then
      self % rttov_opts % cloud_overlap % col_threshold   = 0.05_kind_real ! Threshold for determining if scattering calculations will be performed (RTTOV 12 default was 0.05)
      self % rttov_opts % cloud_overlap % hydro_frac_tlad = .false.

      self % rttov_opts % clw_absorption % clw_data       = .true.  ! Set to true for allocation purposes

      self % rttov_opts % config % check_profiles         = .false. ! we will use the more thorough and verbose user_checkinput
      self % rttov_opts % config % apply_reg_limits       = .true.

      self % rttov_opts % rt_all % use_q2m                = .false. ! for some reason
      self % rttov_opts % rt_all % o3_data                = .true.  ! Set to true for allocation purposes

      self % rttov_opts % scatt % zero_hydro_tlad         = .true.

      self % rttov_opts % surface % mw_sea_emis_model     = mw_emis_model_fastem6  !< Still using Fastem6
    end if
          ...
          do ichan = 1, nchanprof
            ...
            if(chan == self % ch_diags(jvar)) then
              ...
              if(self % ystr_diags(jvar) == var_cli) then
                ...
                nlhs = size(hofxdiags%geovals(jvar)%vals(:,prof))
                nrhs = size(bt_overcast)
                nmin = min(nlhs, nrhs)
                hofxdiags%geovals(jvar)%vals(nlhs-nmin+1:nlhs, prof) = bt_overcast(nrhs-nmin+1:nrhs)
              end if
            end if
          end do
          ...
          do ichan = 1, nchanprof
            ...
            if(chan == self % ch_diags(jvar)) then
              if(self % ystr_diags(jvar) == var_radiance) then
                ...
                hofxdiags%geovals(jvar)%vals(1,prof) = RTProf % emissivity(1) % emis_out(ichan)
              end if
            end if
          end do
          ...
          do ichan = 1, nchanprof
            ...
            if(chan == self % ch_diags(jvar)) then
              if(self % xstr_diags(jvar) == var_ts) then
                ...
                if (conf % do_mw_scatt) then
                    hofxdiags%geovals(jvar)%vals(:,prof) = &
                      RTProf % profiles_k(ichan) % hydro(conf % rttov_hydro_index_ciw, :)
                else
                  hofxdiags%geovals(jvar)%vals(:,prof) = zero
                  if (conf % debug) then
                    message = "ufo_radiancerttov_simobs: Cloud Ice Water only supported for MW scattering"
                    call oops_log%trace(message)
                  end if
                end if
              end if
            ...
            if(chan == self % ch_diags(jvar)) then
              if(self % xstr_diags(jvar) == var_sfc_tskin) then
                hofxdiags%geovals(jvar)%vals(1,prof) = &
                  RTProf % profiles_k(ichan) % skin(1) % t
              else if (self % xstr_diags(jvar) == var_sfc_t2m) then
                hofxdiags%geovals(jvar)%vals(1,prof) = &
                  RTProf % profiles_k(ichan) % near_surface(1) % t2m
              else if (self % xstr_diags(jvar) == var_ps) then
                hofxdiags%geovals(jvar)%vals(1,prof) = &
                  RTProf % profiles_k(ichan) % p(nlevels)
              else if (self % xstr_diags(jvar) == var_sfc_q2m) then
                hofxdiags%geovals(jvar)%vals(1,prof) = &
                  RTProf % profiles_k(ichan) % near_surface(1) % q2m * conf%scale_fac(gas_id_watervapour)
              else if (self % xstr_diags(jvar) == var_sfc_u10) then
                hofxdiags%geovals(jvar)%vals(1,prof) = &
                  RTProf % profiles_k(ichan) % near_surface(1) % wind_u10m
              else if (self % xstr_diags(jvar) == var_sfc_v10) then
                hofxdiags%geovals(jvar)%vals(1,prof) = &
                  RTProf % profiles_k(ichan) % near_surface(1) % wind_v10m
                ...
                hofxdiags%geovals(jvar)%vals(1,prof) = &
                  RTProf % emissivity_k(1) % emis_in(ichan)
              end if
            end if
          end do
...
999      str_pos(1) = index(varstr,jacobianstr) - 1        !position before jacobianstr
  ...
  subroutine ufo_rttov_scale_ozone(self, conf)
    ...
    do iprof = 1, size(self % profiles)
      ...
      call rttov_scale_ref_gas_prof( errorstatus,                            &
                                     conf % rttov_coef_array(inst),          &
  ...
  integer function find_hydrotable_index(RTcoefs, hydro_name) result(idx)
    implicit none
    type(rttov_coefs), intent(in) :: RTcoefs
    character(len=*), intent(in) :: hydro_name
    integer :: i

    idx = -1  ! Default: not found
    do i = 1, RTcoefs % coef_scatt % optp_hydro % ntypes
        if (trim(RTcoefs % coef_scatt % optp_hydro % data(i) % name) == trim(hydro_name)) then
            idx = i
            exit
        end if
    end do
  end function find_hydrotable_index
...
subroutine ufo_rttov_Rad_to_BT(self, cwn, rad, bt)
  ...
  ! Convert radiance (W / (m2 * sr * m-1)) to brightness temperature (K) using Planck function
  bt(1:nchan) = (hplanck*speed_of_light*cwn/kboltz) / log((2.0*hplanck*speed_of_light*speed_of_light*cwn**3) / rad(1:nchan) + 1.0)
