...
ObsLocGC99<MODEL>::ObsLocGC99(const eckit::Configuration & config, const ioda::ObsSpace & obsspace)
...
void ObsLocGC99<MODEL>::computeLocalization(const GeometryIterator_ & i,
  ...
  if ( distName_ != "Halo" && distName_ != "InefficientDistribution" ) {
    std::string message = "Can not use ObsLocGC99 with distribution=" + distName_;
  ...
  if ( options_.searchMethod == SearchMethod::BRUTEFORCE ) {
    ...
    if ( options_.distanceType == DistanceType::CARTESIAN)
      ABORT("ObsLocGC99:: search method must be 'brute_force' when using 'cartesian' distance");
