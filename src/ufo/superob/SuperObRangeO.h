...
#ifndef UFO_SUPEROB_SUPEROBRANGEO_H_
#define UFO_SUPEROB_SUPEROBRANGEO_H_
...
/// \brief Compute the range of non-missing observations in a record.
///
/// For each record consisting of N observations O, a superob S is computed as
/// follows:
///
/// S = max_{k = 1}^{N}(O_k) - min_{k = 1}^{N}(O_k), where O_k != missing.
///
...
class SuperObRangeO : public SuperObBase {
 public:
  using Parameters_ = GenericSuperObParameters;

  explicit SuperObRangeO(const Parameters_ &, const ObsFilterData &,
...
#endif  // UFO_SUPEROB_SUPEROBRANGEO_H_
