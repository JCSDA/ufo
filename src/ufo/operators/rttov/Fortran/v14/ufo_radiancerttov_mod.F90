...
! Add inputs required for MW scattering
...
! Allocate and store names of requested inputs in self%varin for geovals request
    ...
    !Add MW scattering inputs ensuring not to double count duplicates
    if (self % conf % do_mw_scatt) then
      do j = 1, size(varin_scatt)
        if (.not. any(varin_scatt(j) == self%conf%Absorbers)) then
  ...
  subroutine ufo_radiancerttov_simobs(self, geovals, obss, nvars, nlocs,      &
    ...
    call self % RTprof % alloc_profiles(errorstatus, self % conf, nprofiles, nlevels, init=.true., asw=.true.)
    ...
    call oops_log%trace(message)
    call self % RTprof % alloc_direct(errorstatus, self % conf, nprof_sim, nchan_sim, nlevels, init=.true., asw=.true.)
    ...
    if (jacobian_needed) then
      ...
      call self % RTprof % alloc_profiles_k(errorstatus, self % conf, nchan_sim, nlevels, init=.true., asw=.true.)
      call self % RTprof % alloc_k(errorstatus, self % conf, nprof_sim, nchan_sim, nlevels, init=.true., asw=.true.)
    end if
    ...
    RTTOV_loop : do while (prof_start <= prof_end)
      ...
      if (present(ob_info)) then
        if (.not. ob_info % rterror) then
          self % RTprof % emissivity(1) % calc_emis(1:nchan_sim) = ob_info % calc_emiss(:)
          self % RTprof % emissivity(1) % emis_in(1:nchan_sim) = ob_info % emiss(:)
        end if
      else
        if (allocated(sfc_emiss)) then
          self % RTprof % emissivity(1) % calc_emis(:) = .false.
          outerloop: do ichan = 1, ichan_sim  ! list of channels*profiles
            do jchan = 1, self % RTprof % nchan_inst  ! list of self % channels
              ...
              if (self % conf % rttov_coef_array(1) % coef % ff_ori_chn(chanprof(ichan) % chan) == self % channels(jchan)) then
                ...
                self % RTprof % emissivity(1) % emis_in(ichan) = sfc_emiss(jchan, iprof)
                self % RTprof % emissivity(1) % calc_emis(ichan) = .false.
                if (self % RTprof % emissivity(1) % emis_in(ichan) == 0.0) then
                  self % RTprof % emissivity(1) % calc_emis(ichan) = .true.
                end if
                cycle outerloop
              end if
            end do
      ...
      if (do_profile_diagnostics) then
        do ichan = 1, ichan_sim, self % RTprof % nchan_inst
          ...
          if(self % RTProf % print_profile(iprof)) then
            write(*,*) "profile ", iprof
            write(*,*) "calcemiss = ",self % RTprof % emissivity(1) % calc_emis(ichan:ichan+self%RTprof%nchan_inst-1)
            write(*,*) "emissivity in = ",self % RTprof % emissivity(1) % emis_in(ichan:ichan+self%RTprof%nchan_inst-1)
          end if
        end do
      end if
      ...
      if (nchan_sim > 0) then
        if (jacobian_needed) then
          call rttov_k( errorstatus,                                                    &
                        ...
                        self % RTProf % profiles(prof_start:prof_start + nprof_sim -1), &
                        self % RTProf % profiles_k(1:nchan_sim),                        &
                        self % RTProf % emissivity,                                     &
                        self % RTProf % emissivity_k,                                   &
                        self % RTProf % transmission,                                   &
                        self % RTProf % transmission_k,                                 &
                        self % RTProf % radiance,                                       &
                        self % RTProf % radiance_k )
        ...
        if ( errorstatus == errorstatus_success ) then
          ...
          if (do_profile_diagnostics) then
            do ichan = 1, ichan_sim, self % RTprof % nchan_inst
              ...
              if(self % RTProf % print_profile(iprof)) then
                write(*,*) "profile ", iprof
                write(*,*) "emissivity out = ",self % RTprof % emissivity(1) % emis_out(ichan:ichan+self%RTprof%nchan_inst-1)
...
!               end if
    ...
    if(jacobian_needed) then
      call self % RTprof % alloc_k(errorstatus, self % conf, -1, -1, -1, asw=.false.)
      call self % RTprof % alloc_profiles_k(errorstatus, self % conf, size(self % RTprof % profiles_k), -1, asw=.false.)
    ...
    call self % RTprof % alloc_direct(errorstatus, self % conf, -1, -1, -1, asw=.false.)
    call self % RTprof % alloc_profiles(errorstatus, self % conf, size(self % RTprof % profiles), -1, asw=.false.)
