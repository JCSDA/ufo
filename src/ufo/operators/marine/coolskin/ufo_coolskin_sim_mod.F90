...
contains
  ...
  ! local variables
  integer:: N_i,i
  real(kind=kind_real) :: delta,fc,u,lamda,sig,Q0,Qb,Td
  ...
  real(kind=kind_real) :: lamda ,Q0 ,Qb ,Ts ,dTc ,c0 ,y ,Q ,sig
  ...
  Td = Tdc + 273.15  ! convert from C to K
  Ts = Ts + 273.15
  sig = Stefan_Boltzmann_const * 100.0
  ...
  const   = ((alpha*grav/(Rou*cw))*(16.0*Rou**2.0 * cw**2.0 * v_w**3.0 / k_t**2.0))**(0.75)
