...
real(kind_real), parameter           :: Identity(2,2) = RESHAPE ((/ 1, 0, 0, 1 /), SHAPE (Identity))
...
Gap = sqrt (Trace ** 2 - 4 * Determinant)
...
if (Eigenvectors(2,1) ** 2 + Eigenvectors(2,2) ** 2 < Eigenvectors(1,1) ** 2 + Eigenvectors(1,2) ** 2) then
...
Normalisation = sum (FirstEigenvector ** 2) * Eigenvalue(2) * half
