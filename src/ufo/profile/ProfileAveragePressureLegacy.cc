...
#include "ufo/profile/ProfileAveragePressureLegacy.h"

#include "ufo/utils/metoffice/MetOfficeObservationIDs.h"

namespace ufo {

  static ProfileCheckMaker<ProfileAveragePressureLegacy>
  makerProfileAveragePressureLegacy_("AveragePressureLegacy");

  ProfileAveragePressureLegacy::ProfileAveragePressureLegacy
  (const ProfileConsistencyCheckParameters &options)
    : ProfileCheckBase(options)
  {}

  void ProfileAveragePressureLegacy::logPressure(const std::vector <float> &pressures,
  ...
  void ProfileAveragePressureLegacy::ExnerPressure(const std::vector <float> &pressures,
  ...
  void ProfileAveragePressureLegacy::bigPressureGaps(const std::vector <float> &pressures,
                                                     const int ObsType,
  ...
  void ProfileAveragePressureLegacy::runCheck(ProfileDataHandler &profileDataHandler)
  {
    ...
    profileDataHandler.set<float>(ufo::VariableNames::modellevels_logP_rho_derived,
                                  std::move(geovals_logP_rho));
    profileDataHandler.set<float>(ufo::VariableNames::modellevels_logP_derived,
                                  std::move(geovals_logP));
    profileDataHandler.set<float>(ufo::VariableNames::modellevels_ExnerP_rho_derived,
                                  std::move(geovals_ExnerP_rho));
    profileDataHandler.set<float>(ufo::VariableNames::modellevels_ExnerP_derived,
                                  std::move(geovals_ExnerP));
  }

  void ProfileAveragePressureLegacy::fillValidationData(ProfileDataHandler &profileDataHandler)
  {
