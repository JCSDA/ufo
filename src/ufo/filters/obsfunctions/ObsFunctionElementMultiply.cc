...
static ObsFunctionMaker<ElementMultiply<float>> ElementMultiplyfloatMaker(
    "ElementMultiply");
static ObsFunctionMaker<ElementMultiply<int>> ElementMultiplyintMaker(
    "ElementMultiply");

// -----------------------------------------------------------------------------

template <typename FunctionValue>
ElementMultiply<FunctionValue>::ElementMultiply(
...
template <typename FunctionValue>
void ElementMultiply<FunctionValue>::compute(
      ...
      for (size_t ichan = 0; ichan < out.nvars(); ++ichan) {
        if (varin[ichan][iloc] == missing || out[ichan][iloc] == missing) {
          out[ichan][iloc] = missing;
                   ...
                   static_cast<int>(std::round(exponents[ivar])) !=
                       exponents[ivar]) {
...
void ElementMultiply<FunctionValue>::warnOrThrow(const std::string& msg) const {
...
template <typename FunctionValue>
const ufo::Variables& ElementMultiply<FunctionValue>::requiredVariables()
    const {
  return invars_;
}
...
template <typename FunctionValue>
FunctionValue ElementMultiply<FunctionValue>::power(
    FunctionValue value, FunctionValue exponent) const {
