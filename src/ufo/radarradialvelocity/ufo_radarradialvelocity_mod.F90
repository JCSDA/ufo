...
! DONE
 ...
 character(len=maxvarlen), dimension(4), parameter :: varin_default = (/var_u, &
                                                                        var_v, &
                                                                        var_w, &
                                                                        var_refl/)
...
! ------------------------------------------------------------------------------
! Done
subroutine ufo_radarradialvelocity_setup(self, f_conf, vars)
implicit none
...
character(len=MAXVARLEN), dimension(:), intent(inout) :: vars
  ...
  allocate(self%varout(self%nvars_in))
  self%varout = varin_default
...
! Code in this routine is for radarradialvelocity only, please remove and replace
