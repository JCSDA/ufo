...
#ifndef UFO_SUPEROB_SUPEROBCOUNTO_H_
#define UFO_SUPEROB_SUPEROBCOUNTO_H_
...
/// \brief Count the number of non-missing observations in a record.
///
/// For each record consisting of N observations O, a superob S is computed as
/// follows:
///
/// S = count_{k = 1}^{N} (O_k != missing).
///
...
class SuperObCount : public SuperObBase {
 public:
  using Parameters_ = GenericSuperObParameters;
  explicit SuperObCount(const Parameters_ &, const ObsFilterData &,
...
#endif  // UFO_SUPEROB_SUPEROBCOUNTO_H_
