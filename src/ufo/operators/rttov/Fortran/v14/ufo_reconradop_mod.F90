    ...
    do ichan = 1, nchan_sim, rtprof % nchan_inst
      ...
      if (jacobian_needed) then
        ...
        do localchan = 1, rtprof % nchan_inst
          ...
          ! var_sfc_t2m
          self % profile_k_rr_surf(localchan,1) = rtprof % profiles_k(ichan + localchan - 1) % near_surface(1) % t2m
          ! var_sfc_q2m
          self % profile_k_rr_surf(localchan,2) = rtprof % profiles_k(ichan + localchan - 1) % near_surface(1) % q2m
          ! var_sfc_u10
          self % profile_k_rr_surf(localchan,3) = rtprof % profiles_k(ichan + localchan - 1) % near_surface(1) % wind_u10m
          ! var_sfc_u10
          self % profile_k_rr_surf(localchan,4) = rtprof % profiles_k(ichan + localchan - 1) % near_surface(1) % wind_u10m
          ! surf press
          self % profile_k_rr_surf(localchan,5) = rtprof % profiles_k(ichan + localchan - 1) % &
                                                  p_half(ubound(rtprof % profiles_k(1) % p_half, dim=1))
          ! var_sfc_tskin
          self % profile_k_rr_surf(localchan,6) = rtprof % profiles_k(ichan + localchan - 1) % skin(1) % t
          ! var_sfc_emiss
          self % profile_k_rr_surf(localchan,7) = rtprof % emissivity_k(1) % emis_in(ichan + localchan - 1)
          ! ctp
