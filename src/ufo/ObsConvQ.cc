  ...
  : keyOperConvQ_(0), varin_()
{
  ...
  ufo_convq_setup_f90(keyOperConvQ_, &configc);
  int keyVarin;
  ufo_convq_inputs_f90(keyOperConvQ_, keyVarin);
...
ObsConvQ::~ObsConvQ() {
  ufo_convq_delete_f90(keyOperConvQ_);
...
void ObsConvQ::obsEquiv(const GeoVaLs & gom, ObsVector & ovec,
  ...
  ufo_convq_eqv_f90(gom.toFortran(), ovec.toFortran());
}
