...
contains
  ...
  subroutine ufo_radiancerttov_tlad_setup(self, f_confOper, channels)
    ...
    coefloop: do ii = 1, self % RTprof_k % nchan_inst
      jj = jnew
      do while ( jj <= self % conf % rttov_coef_array(1) % coef % fmv_nchn )
  ...
  subroutine ufo_radiancerttov_tlad_settraj(self, geovals, obss, hofxdiags)
    ...
    RTTOV_loop : do while (prof_start <= prof_end)
        ...
        outerloop: do ichan = 1, ichan_sim  ! list of channels*profiles
          do jchan = 1, self % RTprof_K % nchan_inst  ! list of self % channels
            ...
            if (self % conf % rttov_coef_array(1) % coef % ff_ori_chn(chanprof(ichan) % chan) == self % channels(jchan)) then
              ...
              self % RTprof_K % emissivity(1) % emis_in(ichan) = sfc_emiss(jchan, iprof)
              self % RTprof_K % emissivity(1) % calc_emis(ichan) = .false.
              if (self % RTprof_K % emissivity(1) % emis_in(ichan) == 0.0) then
                self % RTprof_K % emissivity(1) % calc_emis(ichan) = .true.
              end if
              cycle outerloop
            end if
          end do
      ...
      if(do_profile_diagnostics) then
        do ichan = 1, ichan_sim, self % RTprof_K % nchan_inst
          ...
          if(self % RTprof_K % print_profile(iprof)) then
            ...
            write(*,*) "tlad settraj calcemiss = ",self % RTprof_K % emissivity(1) % calc_emis(ichan:ichan+self%RTprof_K%nchan_inst-1)
            write(*,*) "tlad settraj emissivity in = ",self % RTprof_K % emissivity(1) % emis_in(ichan:ichan+self%RTprof_K%nchan_inst-1)
          end if
        end do
      end if
                      ...
                      do localchan = 1, self % RTprof_K % nchan_inst
                        if (self % conf % do_mw_scatt) then
                          self % RTprof_K % profiles_k(nchan_total + ichan + localchan - 1) &
                            % hydro(self % conf % rttov_hydro_index_clw, ilev) = zero
                        else
                          self % RTprof_K % profiles_k(nchan_total + ichan + localchan - 1) &
                            % clw(ilev) = zero
                        endif
                  ...
                  do ichan = 1, nchan_sim, self % RTprof_K % nchan_inst
                    ...
                    do ilev = 1, self % nlevels
                      if (self % RTProf_K % ciw_profile_reset(iprof, ilev)) then
                        do localchan = 1, self % RTprof_K % nchan_inst
                          self % RTprof_K % profiles_k(nchan_total + ichan + localchan - 1) &
                                          % hydro(self % conf % rttov_hydro_index_ciw, ilev) = zero
                        end do
                      end if
                    end do
                  end do
      ...
      if(do_profile_diagnostics) then
        do ichan = 1, ichan_sim, self % RTprof_K % nchan_inst
          ...
          write(*,*) "tlad emissivity out = ",self % RTprof_K % emissivity(1) % emis_out(ichan:ichan+self%RTprof_K%nchan_inst-1)
    ...
    call self % RTprof_K % alloc_k(errorstatus, self % conf, -1, -1, -1, asw=.false.)
    call self % RTprof_K % alloc_direct(errorstatus, self % conf, -1, -1, -1, asw=.false.)
    call self % RTprof_K % alloc_profiles(errorstatus, self % conf, -1, -1, asw=.false.)
          ...
          do ichan = 1, self % nchan_total, size(self % channels)
            ...
            do jchan = 1, size(self % channels)
              if (trim(varname) == var_ts) then
                ...
                ! scale_fac = 1 if Jacobian is in kg/kg
                ! scale_fac converts from ppmv to kg/kg if Jacobian wrt ppmv
                hofx(jchan, prof) = hofx(jchan, prof) + &
                ...
                if (self % conf % do_mw_scatt) then
                  hofx(jchan, prof) = hofx(jchan, prof) + &
                    sum(self % RTprof_K % profiles_k(ichan+jchan-1) % &
                    hydro( self % conf % rttov_hydro_index_clw, : ) * &
                    geoval_d % vals(1:geoval_d % nval, prof))
                else
                  hofx(jchan, prof) = hofx(jchan, prof) + &
          ...
          do ichan = 1, self % nchan_total, size(self % channels)
            ...
            do jchan = 1, size(self%channels)
              if (trim(varname) == var_sfc_t2m) then
                ...
                ! scale_fac converts from ppmv to kg/kg if Jacobian wrt ppmv
                hofx(jchan, prof) = hofx(jchan, prof) + &
                  self % RTprof_K % profiles_k(ichan+jchan-1) % near_surface(1) % q2m * &
                  geoval_d % vals(1,prof) * self%conf%scale_fac(gas_id_watervapour)
              else if (trim(varname) == var_sfc_u10) then
                hofx(jchan, prof) = hofx(jchan, prof) + &
                  self % RTprof_K % profiles_k(ichan+jchan-1) % near_surface(1) % wind_u10m * geoval_d % vals(1, prof)
              else if (trim(varname) == var_sfc_v10) then
                hofx(jchan, prof) = hofx(jchan, prof) + &
                  self % RTprof_K % profiles_k(ichan+jchan-1) % near_surface(1) % wind_v10m * geoval_d % vals(1, prof)
              else if (trim(varname) == var_sfc_tskin) then
                hofx(jchan, prof) = hofx(jchan, prof) + &
                  self % RTprof_K % profiles_k(ichan+jchan-1) % skin(1) % t * geoval_d % vals(1, prof)
              else if (trim(varname) == var_sfc_emiss) then
                hofx(jchan, prof) = hofx(jchan, prof) + &
                  self % RTProf_K % emissivity_k(1) % emis_in(ichan+jchan-1) * geoval_d % vals(1, prof)
              end if
            end do
          end do
              ...
              if (hofx(jchan, prof) /= missing) then
                if (trim(varname) == var_ts) then
                  ...
                  if (self % conf % do_mw_scatt) then
                    ...
                    hydro(self % conf % rttov_hydro_index_clw, :) * hofx(jchan, prof)
                  else
                    geoval_d % vals(:, prof) = geoval_d % vals(:, prof) + &
              ...
              if (hofx(jchan, prof) /= missing) then
                if (trim(varname) == var_sfc_t2m) then
                  geoval_d % vals(1, prof) = geoval_d % vals(1, prof) + &
                    self % RTprof_K % profiles_k(ichan+jchan-1) % near_surface(1) % t2m * hofx(jchan, prof)
                else if (trim(varname) == var_sfc_q2m) then
                  ! scale_fac = 1 if Jacobian is in kg/kg
                  ! scale_fac converts from ppmv to kg/kg if Jacobian wrt ppmv
                  geoval_d % vals(1, prof) = geoval_d % vals(1, prof) + &
                  ...
                  geoval_d % vals(1, prof) = geoval_d % vals(1, prof) + &
                    self % RTprof_K % profiles_k(ichan+jchan-1) % near_surface(1) % wind_u10m * hofx(jchan, prof)
                else if (trim(varname) == var_sfc_v10) then
                  geoval_d % vals(1, prof) = geoval_d % vals(1, prof) + &
                    self % RTprof_K % profiles_k(ichan+jchan-1) % near_surface(1) % wind_v10m * hofx(jchan, prof)
                else if (trim(varname) == var_sfc_tskin) then
                  geoval_d % vals(1, prof) = geoval_d % vals(1, prof) + &
                    self % RTprof_K % profiles_k(ichan+jchan-1) % skin(1) % t * hofx(jchan, prof)
                else if (trim(varname) == var_sfc_emiss) then
                  geoval_d % vals(1, prof) = geoval_d % vals(1, prof) + &
                    self % RTProf_K % emissivity_k(1) % emis_in(ichan+jchan-1) * hofx(jchan, prof)
                end if
              end if
            end do
          end do
